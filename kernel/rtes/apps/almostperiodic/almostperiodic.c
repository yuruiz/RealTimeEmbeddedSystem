...
int t, c;
int work = 0;
int bug_timeout = 0, bug_pause = 0;
struct itimerval w_timer, p_timer;

void work_handler (int signum) {
    if(bug_timeout <= 0 && bug_pause <= 0){
        srand(time(NULL));
        int randout = rand();

        bug_timeout = 10 + (10 * (double)randout) / (double)RAND_MAX;
        bug_timeout *= 1000;

        bug_pause = 5 + (5 * (double)randout) / (double)RAND_MAX;
        bug_pause *= 1000;

        //printf("Bug set,pause: %dms, next bug: %dms\n", bug_pause, bug_timeout);
    }
...
int main(int argc, char *argv[]) {
    ...
    syscall(__NR_set_reserve, syscall(__NR_gettid), &T, &T, cpu);
