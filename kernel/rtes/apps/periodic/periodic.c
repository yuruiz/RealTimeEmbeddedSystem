...
void work_handler (int signum) {
    ...
    getitimer(ITIMER_REAL, &timer); /* get time left */
    //printf("Work finished! time left %d sec %d usec \n", timer.it_value.tv_sec, timer.it_value.tv_usec);
    sleep_time.tv_sec = timer.it_value.tv_sec;
    sleep_time.tv_nsec = timer.it_value.tv_usec*1000;
