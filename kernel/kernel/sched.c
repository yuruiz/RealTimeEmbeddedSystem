...
#include <linux/reserve.h> /* 18-648 reserve function */
...
need_resched:
	...
	if (likely(prev != next)) {
        ...
        /* disable c timer before swap out */
        if (prev->reserve.flag) {
            hrtimer_cancel(&(prev->reserve.c_timer));
        }

        /* calculate energy */
        if (prev->reserve.tracking_energy) {
            unsigned int freq = cpu_freq[prev->reserve.cpu];
            unsigned int power = 0;
            if (freq >= MIN_FREQ && freq <= MAX_FREQ) {
                power = freq_to_power[cpu_freq[prev->reserve.cpu] - MIN_FREQ];
            }
            unsigned long increment = power * prev->reserve.acc_single_run.tv_sec + power * (prev->reserve.acc_single_run.tv_nsec / NS_PER_MILLI_S) / MILLI_S_PER_S;
            prev->reserve.energy += increment;
            spin_lock_irqsave(&total_energy_lock, flags);
            total_energy += increment;
            spin_unlock_irqrestore(&total_energy_lock, flags);

            printk("team6: running end: %d sec %d nsec \n", (int)prev->reserve.acc_single_run.tv_sec, (int)prev->reserve.acc_single_run.tv_nsec);
            printk("team6: cpu%d: freq: %u power: %u\n", prev->reserve.cpu, cpu_freq[prev->reserve.cpu], power);
            printk("team6: new energy: %lu\n", power * prev->reserve.acc_single_run.tv_sec + power * (prev->reserve.acc_single_run.tv_nsec / NS_PER_MILLI_S) / MILLI_S_PER_S);
            printk("team6: total energy: %lu\n", prev->reserve.energy);
    }

        /* reset acc_single_run counter */
        prev->reserve.acc_single_run.tv_nsec = 0;
        prev->reserve.acc_single_run.tv_sec = 0;
        //printk("team6: reset time: %d sec %d nsec \n\n", (int) prev->reserve.acc_single_run.tv_sec, (int)prev->reserve.acc_single_run.tv_sec);

		context_switch(rq, prev, next); /* unlocks the rq */

        /* enable c timer after swap in */
        if (prev->reserve.flag) {
            /* c timer */
            ktime_t ktime = ktime_set(0, RESERVE_INTERVAL_NS);
            hrtimer_start(&(prev->reserve.c_timer), ktime, HRTIMER_MODE_REL | HRTIMER_MODE_PINNED);
            
            /* p timer */
            /*
            if (prev->reserve.p_start == 0) {
                prev->reserve.p_start = 1;
                ktime = ktime_set(prev->reserve.T.tv_sec, prev->reserve.T.tv_nsec);
                hrtimer_start(&(prev->reserve.p_timer), ktime, HRTIMER_MODE_REL | HRTIMER_MODE_PINNED);
            }
            */
        }
...
long sched_setaffinity(pid_t pid, const struct cpumask *in_mask)
{
	...
	retval = -EPERM;
        ...
        /* 18-648 Labs: skip permission check */
/*
